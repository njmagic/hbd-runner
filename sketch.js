...
function hideLeaderboardForm() {
  ...
  let finalState = gameState; // Capture current state

  // If we were entering score, the state WILL become gameOver
  if (gameState === "enteringScore") {
    finalState = "gameOver"; // Determine the state *after* this function finishes
    gameState = "gameOver"; // Update the actual state
  }
  
  // --- Mobile Button Visibility --- 
  if (isMobileDevice) {
      // Show restart button if final state is GameOver or Leaderboard view
      if (restartBtn && (finalState === 'gameOver' || finalState === 'leaderboard')) {
